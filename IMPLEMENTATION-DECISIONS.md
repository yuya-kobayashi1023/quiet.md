...
| AUTO-043 | ピン止めは Notes と Archive の両方で有効であり、Recent には表示しない。Archive と同様に相対パスで保持し、Rename 時は新しい相対パスへ追従する。 | Low |
...
| AUTO-055 | 一括操作の対象は、その操作で実際に状態が変わる行のみとする。アーカイブ済みと未アーカイブが混在した選択で「アーカイブ」を実行した場合、対象は未アーカイブの行のみとし、Toast の件数と「元に戻す」の対象もその一覧とする。ピン止めおよびその解除も同様とする。 | Low |
