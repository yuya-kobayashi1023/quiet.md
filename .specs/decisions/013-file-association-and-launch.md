...
- 表示件数は設定で変えられる（`recentVisibleCount`、既定 8）。画面の高さで妥当な数が変わるため
  （2026-09-09 改定。下の Amendment を参照）
...
## Amendment（2026-09-09）— Recent の位置と件数

ADR-016 で Sidebar の構造が変わり、Workspace が最上部へ移った。その流れで Recent 側も見直した。

### 1. Sidebar の下端へ固定する

Notes / Archive と同じスクロール領域に置くと、Notes の増減で Recent の位置が動く。
「いつも同じ場所にある」ことが Recent の価値なので、スクロールの外へ出して下端に固定する。

### 2. 既定は畳んでおく

見出しごと disclosure にして、既定は閉じた状態にする。Recent は「たまに前のファイルへ戻る」
ための導線であって、書いている間ずっと見えている必要はない。開いていると Notes より下が
賑やかになり、視線が散る。

開閉は Sidebar のセッション内でだけ覚える（設定にも `.quiet/workspace.json` にも書かない）。
起動のたびに閉じた状態へ戻るのは意図した挙動で、「既定は畳んである」を毎回の起動で満たす。

### 3. 開いたときの表示は 10 件 + `More`

`recentVisibleCount`（既定 8、Settings で 3–30）は削除する。

- 高さの上限をリスト側（10 行ぶん）で持つようになり、件数を設定で変える理由が消えた
- 30 件全部を見たい場面はあるが、常時見せる必要はない。`More` で開き、
  開いたぶんは 10 行の高さの中でスクロールする
- `AGENTS.md` §4 と同じ理由で、意味の薄くなった設定項目は残さない
