...
### 7. セクション見出しの `+` で Workspace を開く

Notes の「新規ノート」と同じ位置・同じ見た目の `+` を置き、フォルダ選択ダイアログを開く。
Sidebar の他のセクション（Archive / Recent）に常設ボタンは無いが、ここだけは
「一覧に無いフォルダを足す」という、そのセクション自身に対する操作になるため例外とする。

履歴が空でもセクションを出す。空のときこそ `+` が要る（`ui-spec.md` §2 の Recent とは異なる）。
